    ...
    for n in G.nodes():
        ...
        for i in range(num_int):
            if(i!=0):
                ...
                z=float(n)+0.1
            G.add_node(z)  #attenzione , id nodi aggiunti e' un int, ad esempio 4.1 e non '4.1'
            elem.append(z)
...
def CalcolaCentralitaGrafoEspanso(G,numnodiorginale):
    BCprimo = BC_salta_nodi(G, aggiunti,weight='weight',endpoints=False,normalized=False)
    
    nodi = G.nodes()
    for i in set(aggiunti):
        nodi.remove(i)

    BCsecondo = BC_salta_nodi(G, nodi,weight='weight',endpoints=False,normalized=False)
...
def Genera_e_Salva():
    ...
    #ATTENZIONE! id nodi e' ad es '4' e NON 4. Graphml salva id come stringa
    SalvaInGraphml(g)
    #nx.draw(g, with_labels=True)
    #plt.show()
...
def Run_Test():
    ...
    BC_origin= nx.betweenness_centrality(grafo, weight='weight',endpoints=True,normalized=False)     
